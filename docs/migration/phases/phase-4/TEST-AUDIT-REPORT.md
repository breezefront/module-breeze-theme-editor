...
**Дата:** 24 лютого 2026  
**Статус:** ✅ PHP Validated | ✅ Admin JS Validated | 📋 Frontend JS — pending migration  
...
| **Admin JS** | 14 | 126 | 126 | 0 | 0 | ✅ PASS |
| **Frontend JS** | 22 | ~170 | — | — | — | 📋 Pending migration |
| **TOTAL** | **37** | **416** | **414** | **0** | **2** | ✅ |

> **Примітка щодо Frontend JS:** Поточна фронтенд реалізація застаріла і не підтримується. Тести **не видаляти** — вони мігруються разом з функціоналом у міру перенесення.
...
**Підсумок Admin JS:** **126/126 pass** — підтверджено браузером 24.02.2026.

### Frontend JS Tests (22 модулі зареєстровано)

**Статус:** 📋 **Pending migration — тести мігруються разом з функціоналом**

Поточна фронтенд реалізація застаріла. Тести **зберігаються** у `view/frontend/web/js/test/tests/` і будуть мігровані по мірі перенесення відповідного функціоналу. Кожен тест переноситься разом з модулем, який він тестує.
...
### Обов'язково
1. **Frontend JS тести** — мігруються по мірі перенесення функціоналу (не одноразова задача)
...
- [x] Admin JS тести запущені в браузері: **126/126 pass** — 24.02.2026
- [x] Frontend JS — стратегія визначена: мігруються разом з функціоналом
